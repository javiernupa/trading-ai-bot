"""Estrategia basada en Media Móvil de 50 períodos (MA50).

La MA50 es un indicador de tendencia a medio plazo:
- Precio por encima de MA50 → Tendencia alcista → COMPRAR
- Precio por debajo de MA50 → Tendencia bajista → VENDER
- MA50 actúa como soporte/resistencia dinámica

Esta estrategia es ideal para:
- Trading de tendencias a medio plazo
- Identificar cambios de tendencia más rápido que MA200
- Swing trading
"""
...
class Ma50Strategy(Strategy):
    """Estrategia de Media Móvil de 50 períodos.

    Señales:
    - COMPRA (1): Precio cruza por encima de MA50
    - VENTA (-1): Precio cruza por debajo de MA50
    - MANTENER (0): Sin cruce

    Además, opcionalmente puede usar:
    - Filtro de tendencia: Solo operar cuando precio está por encima/debajo de MA50
    ...
    def __init__(
        ...
        """Inicializa la estrategia MA50.

        Args:
            period: Período de la media móvil (default: 50)
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        """Genera señales de trading basadas en MA50.
        ...
        # Calcular MA50
        df["ma50"] = df["close"].rolling(window=self.period).mean()

        # Calcular posición relativa del precio vs MA50
        df["price_above_ma"] = (df["close"] > df["ma50"]).astype(bool)
        ...
        # Eliminar señales donde MA50 no está calculada aún
        df.loc[df["ma50"].isna(), "signal"] = 0
    ...
    def get_indicator_values(self, data: pd.DataFrame) -> dict:
        ...
        if result["ma50"] and result["current_price"]:
            distance = (
                (result["current_price"] - result["ma50"]) / result["ma50"] * 100
            )
    ...
    def __str__(self) -> str:
        ...
        return f"Ma50Strategy(period={self.period}, mode={mode}{volume})"
